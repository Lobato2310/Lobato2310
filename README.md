...
Atuar como **Analista de Dados Jr** ou **Cientista de Dados Jr**, com foco em transformar dados em decisões.  
Interesse de longo prazo em **Engenharia de Dados** e sistemas inteligentes aplicados ao setor corporativo.
...
- Pipeline completo: EDA → Pré-processamento → Modelagem → Deploy
- `ColumnTransformer` + `RandomizedSearchCV` para otimização de hiperparâmetros
...
> Aplicação web Django com regras de negócio corporativas reais

- Autenticação com perfis de acesso (Colaborador, Gestor, Admin)
...
- Solução de bypass de CAPTCHA com **Selenium**
- Interface interativa com **Streamlit** integrada à **API da OpenAI** para extração inteligente de dados
- Dados coletados: preço, endereço, tamanho, quartos, banheiros e imagens
- Stack: `Scrapy` · `Selenium` · `BeautifulSoup` · `Streamlit` · `OpenAI API`
